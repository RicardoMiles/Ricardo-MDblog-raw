...
# Run Code in IDE
Run CPP code needs to compile it to machine code that can be run by the computer operating system.
...
# Content included in Mosh's Course
## Basic 
...
## Naming convention
Snake Case, low case word with lower slash
Pascal Case, Capitalized the variable name
Camel Case, first lower case and rest word capitalised
...
### What is the difference between `x++`,`++x`and `x=x+1`
Only decrement and increment have notations like `x++` and `x--`
They can be applied as a postfix or as a prefix
E.g. `++x` `x++`
...
```CPP
 ...
 int z = ++x
```
Use increment operator as a prefix, first step variable x will be incremented by one, and then the value of x will be stored in z
...
If we don't type `f` or `l` after float and long type variable‘s assignment value, compiler will treat them as double and int by default
